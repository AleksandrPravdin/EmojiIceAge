...
public class GameWindow extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        soundOn = settings.getInt("sound", 1);
        if (soundOn == 1) {
    ...
    private void createMainLayout() {
        ...
        buttonSound = findViewById(R.id.sound);
        if (soundOn == 1) {
    ...
    private void change2PlayField() {

        for (int i = playerX - 1; i <= playerX + 1; i++) {
            for (int j = playerY - 1; j <= playerY + 1; j++) {
                if ((j == playerY - 1 && i == playerX - 1) || (j == playerY - 1 && i == playerX + 1) || (j == playerY + 1 && i == playerX - 1) || (j == playerY + 1 && i == playerX + 1)) {
                } else {
